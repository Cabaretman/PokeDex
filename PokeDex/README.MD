...
Below is a breakdown of the app’s functionality, architectural choices, and how each milestone was implemented. I have structured this to roughly follow the milestones I was given, and explain how each individual milestone was implemented
...
The search bar is not ideal. Since the milestones required pagination, and I was doing client side filtering of search results, it only searches through the Pokemon you've actually loaded in. This is fixed by just loading in all of them; the data is so small that this is fairly trivial on most modern devices for this use case at least. However this contradicts one of the milestones I was given with regards to pagination. I decided for now not to violate the given instructions in pursuit of extra credit kudos. As far as I can tell, there is no way to do a search server-side with the PokeAPI.
...
All of these are changes I'd make for a more long term, serious version of this project. For MVP of a take home initial assignment, I'd say this is a good start 
